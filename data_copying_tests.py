...
def C_T(Pn, Pn_cells, Qm, Qm_cells, T, T_cells, tau):
    ...
    m = Qm.shape[0] #assigns # of rows of Q to m
    n = Pn.shape[0]
    ...
    Qm_of_pi = Qm_cts / m # number of occurences of each unique label divided by # of rows of Q
    #print("Qm(pi): ", Qm_of_pi)
    Pi_tau = Qm_of_pi > tau #binary array selecting which cells have sufficient samples 
    #print("Pi_tau (which cells have sufficient Q samples): ", Pi_tau)
    if all(not value for value in Pi_tau):
        print("wARNING: NOT ENOUGH GENERATED SAMPLES IN Q, RESULTS WILL BE INACCURATE")

    #Get the fraction of test samples in each cell Pn(pi) 
    labels, cts = np.unique(Pn_cells, return_counts = True) #labels, cts stores all unique cell labels in Pn_cells and their corresponding counts 
    k = int(np.max(labels)) + 1
    Pn_cts = np.zeros(k) #length of Pn_cts array is max label value + 1
    #print("Pn_cts shape:", Pn_cts.shape)
    #print("labels shape:", labels.shape)
    #print("Pn_cts values:", Pn_cts)
    #print("labels values:", labels)
    Pn_cts[labels.astype(int)] = cts #insert counts in order of cell label 
    Pn_of_pi = Pn_cts / n #array of fraction of test samples in each cell
